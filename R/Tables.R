...
rowDataTable <- function(exp) {
    ...
    x <- rowData(exp)[, cols]
    cols <- vapply(x, function(z) "numeric" %in% class(z), logical(1))
...
replicateTable <- function(input, exp) {
    req(!is.null(exp))
    if (!"Sample" %in% colnames(colData(exp))) {
        return(NULL)
    }
